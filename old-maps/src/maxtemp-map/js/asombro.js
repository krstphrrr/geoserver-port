...
*/
  ...
  var maxTempAnn = L.tileLayer.wms('https://jornada.nmsu.edu/geoserver/jornada/wms', {
    layers: 'jornada:maxtempbycounty_wgs84',
    format: 'image/png',
    transparent: true,
    tiled: true,
    version: '1.3.0',
  ...
  var opaVar = [maxTempAnn];
  var infoObj = { "maxtempbycounty_wgs84": "Max Annual Temp-County"};
  var infoIDField = { "maxtempbycounty_wgs84": "d_tmax_ann"};
  ...
  var overlays = { "Change in Max Temp": maxTempAnn};
...
*/
  ...
  d3.select("#infoDiv")
    .append("div")
    .attr("id", "infoCtls")
    .html('<div id="infoClear" class="infoCtls" title="Clear selected counties">Clear</div><div id="infoDownload" class="infoCtls" title="Download data for selected counties to a CSV file"><a id="infoDownloadA" href="#" download="max_temp.csv">Download</a></div>');
  ...
  d3.select("#legendDiv")
    ...
    .attr("id", "legendTitle")
    .append("span")
    .html('<span class="fa fa-info-circle" data-toggle="tooltip" data-container="body" data-placement="auto" data-html="true" title="<p><u><b>Legend</b></u></p><p>Displays the legend for the predicted change in maximum temperature enabling interpretation along with control over the transparency of the layer.</p>"</span>');
  ...
  function addLegendImg(tmpName, tmpTitle, tmpLayer, tmpPath) {
    ...
    $("#" + tmpName + "LegendImg").one("load", function() {
      ...
      $(d3.select("#maxtempbycounty_wgs84LegendHeader").select("div")._groups[0][0]).trigger("click"); 
  ...
  //******Read in json file
  d3.json("max/gis/maxTemp_simp.json").then(function(data) {
    tmpTopo = topojson.feature(data, data.objects.maxTemp);
    addTopo(tmpTopo);
  });
...
function addTopo(topo) {
  ...
  d3.select("#maxtempbycounty_wgs84G").selectAll(".maxTemp")
    .data(topo.features)
    .enter().append("path")
      .attr("d", path)
      .attr("class", "maxTemp activeTopo")
      .property("data-tt", function(d) { return d.properties.CountyName + ", " + d.properties.State_Abb + ": " + d.properties.D_Tmax_ANN.toFixed(1) + " C"; })
...
function addInfo(d) {
  ...
  d3.select("#tab_" + d.FIPS)
    .append("tr")
    .html('<td>Winter</td><td>' + d.Tmax_DJF.toFixed(1) + '</td><td>' + d.F_Tmax_DJF.toFixed(1) + '</td><td>' + d.D_Tmax_DJF.toFixed(1) + '</td>');
  d3.select("#tab_" + d.FIPS)
    .append("tr")
    .html('<td>Spring</td><td>' + d.Tmax_MAM.toFixed(1) + '</td><td>' + d.F_Tmax_MAM.toFixed(1) + '</td><td>' + d.D_Tmax_MAM.toFixed(1) + '</td>');       
  d3.select("#tab_" + d.FIPS)
    .append("tr")
    .html('<td>Summer</td><td>' + d.Tmax_JJA.toFixed(1) + '</td><td>' + d.F_Tmax_JJA.toFixed(1) + '</td><td>' + d.D_Tmax_JJA.toFixed(1) + '</td>');
  d3.select("#tab_" + d.FIPS)
    .append("tr")
    .html('<td>Fall</td><td>' + d.Tmax_SON.toFixed(1) + '</td><td>' + d.F_Tmax_SON.toFixed(1) + '</td><td>' + d.D_Tmax_SON.toFixed(1) + '</td>');
  d3.select("#tab_" + d.FIPS)
    .append("tr")
    .html('<td>Annual</td><td>' + d.Tmax_ANN.toFixed(1) + '</td><td>' + d.F_Tmax_ANN.toFixed(1) + '</td><td>' + d.D_Tmax_ANN.toFixed(1) + '</td>');
...
function addCSV() {
  ...
  d3.selectAll(".svgSelected").data().forEach(function(d) {
    tmpCSV += d.properties.CountyName + "," + d.properties.StateName + "," + "Winter" + "," +  + d.properties.Tmax_DJF + "," + d.properties.F_Tmax_DJF + "," + d.properties.D_Tmax_DJF + "\r\n";
    tmpCSV += d.properties.CountyName + "," + d.properties.StateName + "," + "Spring" + "," +  + d.properties.Tmax_MAM + "," + d.properties.F_Tmax_MAM + "," + d.properties.D_Tmax_MAM + "\r\n";
    tmpCSV += d.properties.CountyName + "," + d.properties.StateName + "," + "Summer" + "," +  + d.properties.Tmax_JJA + "," + d.properties.F_Tmax_JJA + "," + d.properties.D_Tmax_JJA + "\r\n";
    tmpCSV += d.properties.CountyName + "," + d.properties.StateName + "," + "Fall" + "," +  + d.properties.Tmax_SON + "," + d.properties.F_Tmax_SON + "," + d.properties.D_Tmax_SON + "\r\n";
    tmpCSV += d.properties.CountyName + "," + d.properties.StateName + "," + "Annual" + "," +  + d.properties.Tmax_ANN + "," + d.properties.F_Tmax_ANN + "," + d.properties.D_Tmax_ANN + "\r\n";
  });
