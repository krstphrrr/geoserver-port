...
*/
  ...
  var precipAnn = L.tileLayer.wms('https://jornada.nmsu.edu/geoserver/jornada/wms', {
    layers: 'jornada:precipitationbycounty_wgs84',
    format: 'image/png',
    transparent: true,
    tiled: true,
    version: '1.3.0',
  ...
  var opaVar = [precipAnn];
  var infoObj = { "precipitationbycounty_wgs84": "Annual Precip-County"};
  var infoIDField = { "precipitationbycounty_wgs84": "d_pr_ann"};
  ...
  var overlays = { "Change in Precip": precipAnn};
...
*/
  ...
  d3.select("#infoDiv")
    .append("div")
    .attr("id", "infoCtls")
    .html('<div id="infoClear" class="infoCtls" title="Clear selected counties">Clear</div><div id="infoDownload" class="infoCtls" title="Download data for selected counties to a CSV file"><a id="infoDownloadA" href="#" download="precip.csv">Download</a></div>');
  ...
  d3.select("#legendDiv")
    ...
    .attr("id", "legendTitle")
    .append("span")
    .html('<span class="fa fa-info-circle" data-toggle="tooltip" data-container="body" data-placement="auto" data-html="true" title="<p><u><b>Legend</b></u></p><p>Displays the legend for the predicted change in annual precipitation enabling interpretation along with control over the transparency of the layer.</p>"</span>');
  ...
  function addLegendImg(tmpName, tmpTitle, tmpLayer, tmpPath) {
    ...
    $("#" + tmpName + "LegendImg").one("load", function() {
      ...
      $(d3.select("#precipitationbycounty_wgs84LegendHeader").select("div")._groups[0][0]).trigger("click"); 
  ...
  //******Read in json file
  d3.json("precip/gis/precip_simp.json").then(function(data) {
    tmpTopo = topojson.feature(data, data.objects.precip);
    addTopo(tmpTopo);
  });
...
function addTopo(topo) {
  ...
  d3.select("#precipitationbycounty_wgs84G").selectAll(".precip")
    .data(topo.features)
    .enter().append("path")
      .attr("d", path)
      .attr("class", "precip activeTopo")
      .property("data-tt", function(d) { return d.properties.CountyName + ", " + d.properties.State_Abb + ": " + d.properties.D_PR_ANN.toFixed(1) + " mm"; })
...
function addInfo(d) {
  ...
  d3.select("#tab_" + d.FIPS)
    .append("tr")
    .html('<td>Winter</td><td>' + d.PPT_DJF.toFixed(1) + '</td><td>' + d.F_PR_DJF.toFixed(1) + '</td><td>' + d.D_PR_DJF.toFixed(1) + '</td>');
  d3.select("#tab_" + d.FIPS)
    .append("tr")
    .html('<td>Spring</td><td>' + d.PPT_MAM.toFixed(1) + '</td><td>' + d.F_PR_MAM.toFixed(1) + '</td><td>' + d.D_PR_MAM.toFixed(1) + '</td>');       
  d3.select("#tab_" + d.FIPS)
    .append("tr")
    .html('<td>Summer</td><td>' + d.PPT_JJA.toFixed(1) + '</td><td>' + d.F_PR_JJA.toFixed(1) + '</td><td>' + d.D_PR_JJA.toFixed(1) + '</td>');
  d3.select("#tab_" + d.FIPS)
    .append("tr")
    .html('<td>Fall</td><td>' + d.PPT_SON.toFixed(1) + '</td><td>' + d.F_PR_SON.toFixed(1) + '</td><td>' + d.D_PR_SON.toFixed(1) + '</td>');
  d3.select("#tab_" + d.FIPS)
    .append("tr")
    .html('<td>Annual</td><td>' + d.PPT_ANN.toFixed(1) + '</td><td>' + d.F_PR_ANN.toFixed(1) + '</td><td>' + d.D_PR_ANN.toFixed(1) + '</td>');
...
function addCSV() {
  ...
  d3.selectAll(".svgSelected").data().forEach(function(d) {
    tmpCSV += d.properties.CountyName + "," + d.properties.StateName + "," + "Winter" + "," +  + d.properties.PPT_DJF + "," + d.properties.F_PR_DJF + "," + d.properties.D_PR_DJF + "\r\n";
    tmpCSV += d.properties.CountyName + "," + d.properties.StateName + "," + "Spring" + "," +  + d.properties.PPT_MAM + "," + d.properties.F_PR_MAM + "," + d.properties.D_PR_MAM + "\r\n";
    tmpCSV += d.properties.CountyName + "," + d.properties.StateName + "," + "Summer" + "," +  + d.properties.PPT_JJA + "," + d.properties.F_PR_JJA + "," + d.properties.D_PR_JJA + "\r\n";
    tmpCSV += d.properties.CountyName + "," + d.properties.StateName + "," + "Fall" + "," +  + d.properties.PPT_SON + "," + d.properties.F_PR_SON + "," + d.properties.D_PR_SON + "\r\n";
    tmpCSV += d.properties.CountyName + "," + d.properties.StateName + "," + "Annual" + "," +  + d.properties.PPT_ANN + "," + d.properties.F_PR_ANN + "," + d.properties.D_PR_ANN + "\r\n";
  });
